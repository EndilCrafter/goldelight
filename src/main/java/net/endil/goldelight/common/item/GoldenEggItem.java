...
public class GoldenEggItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        if (!pLevel.isClientSide) {
            ThrownGoldenEgg thrownGoldenEgg = new ThrownGoldenEgg(pLevel, pPlayer);
            thrownGoldenEgg.setItem(itemstack);
            thrownGoldenEgg.shootFromRotation(pPlayer, pPlayer.getXRot(), pPlayer.getYRot(), 0.0F, 1.5F, 1.0F);
            pLevel.addFreshEntity(thrownGoldenEgg);
        }
