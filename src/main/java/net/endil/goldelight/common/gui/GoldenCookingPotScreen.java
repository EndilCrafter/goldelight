...
public class GoldenCookingPotScreen extends AbstractContainerScreen<GoldenCookingPotMenu> implements RecipeUpdateListener {
    private static final ResourceLocation RECIPE_BUTTON_LOCATION = new ResourceLocation("textures/gui/recipe_button.png");
    private static final ResourceLocation BACKGROUND_TEXTURE = new ResourceLocation(GolDelight.MOD_ID, "textures/gui/golden_cooking_pot.png");
    private static final Rectangle HEAT_ICON = new Rectangle(47, 55, 17, 15);
    private static final Rectangle PROGRESS_ARROW = new Rectangle(89, 25, 0, 17);

    private final GoldenCookingPotRecipeBookComponent recipeBookComponent = new GoldenCookingPotRecipeBookComponent();
    private boolean widthTooNarrow;

    public GoldenCookingPotScreen(GoldenCookingPotMenu screenContainer, Inventory inv, net.minecraft.network.chat.Component titleIn) {
    ...
    private void renderHeatIndicatorTooltip(GuiGraphics gui, int mouseX, int mouseY) {
        if (this.isHovering(HEAT_ICON.x, HEAT_ICON.y, HEAT_ICON.width, HEAT_ICON.height, mouseX, mouseY)) {
            String key = "container.golden_cooking_pot." + (this.menu.isHeated() ? "heated" : "not_heated");
            gui.renderTooltip(this.font, GDTextUtils.getTranslation(key, menu), mouseX, mouseY);
        }
    }
    ...
    protected void renderMealDisplayTooltip(GuiGraphics gui, int mouseX, int mouseY) {
        if (this.minecraft != null && this.minecraft.player != null && this.menu.getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
            if (this.hoveredSlot.index == 6) {
                ...
                tooltip.add(GDTextUtils.getTranslation("container.golden_cooking_pot.served_on", container).withStyle(ChatFormatting.GRAY));
